...
* El nuevo servidor clon Ubuntu con Nginx configurado como proxy inverso
...
!!!Info "Clonación MV con Vagrant"
    Para hacer la clonación desde Vagrant podéis ejecutar los siguientes comandos:
        1. Ve al directorio de la MV (donde está el archivo Vagrantfile)
        2. Ejecuta `vagrant package` para exportar la MV a un archivo package.box
        3. Ejecuta `vagrant box add serv-nginx package.box` para añadir la MV exportada a la lista de boxes.
        4. Ejecuta `vagrant box list` para verificar que se ha añadido.
        5. Crea una nueva carpeta para la nueva MV y ejecuta `vagrant init` estableciendo como box `config.vm.box = "serv-nginx"` y añade también la siguiente linea: `config.vm.network "public_network", type: "dhcp"` para que se le asigne una IP.
