...
Vamos a cambiar el nombre que tuviera vuestra web por el de `webserver`, para ello: 
  ...
  * Cambiad el nombre del archivo de configuración del sitio, es decir, el fichero `ejemplo2` por `webserver`.
...
+ Cread un archivo de configuración en *sites-available* con el nombre `ejemplo-proxy`.
