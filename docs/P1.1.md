...
Para que Nginx presente el contenido de nuestra web, es necesario crear un bloque de servidor con las directivas correctas. En vez de modificar el archivo de configuración predeterminado directamente, crearemos uno nuevo en `/etc/nginx/sites-available/ejemplo2`: 

```console
sudo nano /etc/nginx/sites-available/ejemplo2 
```
...
```console
sudo ln -s /etc/nginx/sites-available/ejemplo2 /etc/nginx/sites-enabled/
```
